import React from 'react'
import { Link } from 'react-router-dom'

const Home = () => {
  return (
    <div
      style={{
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'center',
        alignItems: 'center',
      }}
    >
      <Link to="/nl_nl_test_set/guiding_principles">nl nl test set</Link>
      <Link to="/uk_ua_test_set/guiding_principles">uk ua test set</Link>
      <Link to="/nepali/guiding_principles">Nepali</Link>
      <Link to="/armenian/guiding_principles">Armenian</Link>
      <Link to="/bangla/guiding_principles">Bangla</Link>
      <Link to="/belarusian/guiding_principles">Belarusian</Link>
      <Link to="/burmese/guiding_principles">Burmese</Link>
      <Link to="/dutch/guiding_principles">Dutch</Link>
      <Link to="/filipino/guiding_principles">Filipino</Link>
      <Link to="/georgian/guiding_principles">Georgian</Link>
      <Link to="/greek/guiding_principles">Greek</Link>
      <Link to="/gujarati/guiding_principles">Gujarati</Link>
      <Link to="/hindi/guiding_principles">Hindi</Link>
      <Link to="/indonesian/guiding_principles">Indonesian</Link>
      <Link to="/italian/guiding_principles">Italian</Link>
      <Link to="/japanese/guiding_principles">Japnsese</Link>
      <Link to="/javanese/guiding_principles">Javanese</Link>
      <Link to="/kannada/guiding_principles">Kannada</Link>
      <Link to="/korean/guiding_principles">Korean</Link>
      <Link to="/macedonian/guiding_principles">Macedonian</Link>
      <Link to="/malay/guiding_principles">Malay</Link>
      <Link to="/malayalam/guiding_principles">Malayalam</Link>
      <Link to="/mandarin/guiding_principles">Mandarin</Link>
      <Link to="/marathi/guiding_principles">Marathi</Link>
      <Link to="/mongolian/guiding_principles">Mongolian</Link>
      <Link to="/noreweign/guiding_principles">Noreweign</Link>
      <Link to="/pujabi/guiding_principles">Pujabi</Link>
      <Link to="/russian/guiding_principles">Russian</Link>
      <Link to="/slovak/guiding_principles">Slovak</Link>
      <Link to="/slovene/guiding_principles">Slovene</Link>
      <Link to="/sundanese/guiding_principles">Sundanese</Link>
      <Link to="/swahili/guiding_principles">Swahili</Link>
      <Link to="/swedish/guiding_principles">Swedish</Link>
      <Link to="/tamil/guiding_principles">Tamil</Link>
      <Link to="/telugu/guiding_principles">Telugu</Link>
      <Link to="/thai/guiding_principles">Thai</Link>
      <Link to="/turskish/guiding_principles">Turskish</Link>
      <Link to="/ukraninan/guiding_principles">Ukraninan</Link>
      <Link to="/urdu/guiding_principles">Urdu</Link>
      <Link to="/vietnamese/guiding_principles">Vietnamese</Link>
    </div>
  )
}

export default Home
